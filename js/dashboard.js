...
// ============================================================
// NAVIGATION — full lazy-render wiring for every page
// ============================================================
function navigateTo(pageId) {
  ...
  // ---- LAZY CHART RENDER PER PAGE ----
  // Each block always calls destroyChart internally (via each renderChart fn),
  // so re-navigating re-renders fresh. Guards via CHARTS registry avoid
  // double-init on first load since initCharts() already ran.

  switch (pageId) {
    case 'dashboard':
      if (!CHARTS.tren)      renderChartTren();
      if (!CHARTS.bor)       renderChartBOR();
      if (!CHARTS.statusKab) renderChartStatusKab();
      break;
...
function renderGiziTable() {
  ...
  tbody.innerHTML = SIKESPA_DATA.gizi.map(g => {
    const colorVar = g.status === 'critical' ? 'notification' : g.status === 'warning' ? 'warning' : 'success';
    return `<tr>
      <td><strong>${g.kabupaten}</strong></td>
      <td class="col-num" style="font-weight:700;color:var(--color-${colorVar})">${g.stunting}%</td>
...
function renderNakesTable() {
  ...
  tbody.innerHTML = SIKESPA_DATA.nakes.map(n => {
    ...
    const gapColor = gap > 20 ? 'notification' : gap > 0 ? 'warning' : 'success';
    return `<tr>
      ...
      <td class="col-num" style="color:var(--color-${gapColor});font-weight:700">${gap > 0 ? '-' + gap : '✔'}</td>
    </tr>`;
  }).join('');
}

// ============================================================
// KIA SUMMARY CARDS — fixed icons
// ============================================================
function renderKIASummary() {
  ...
  const items = [
    { key: 'akiProvinsi',     icon: '🩸', higherIsBetter: false, desc: 'Angka Kematian Ibu' },
    { key: 'akbProvinsi',     icon: '👶', higherIsBetter: false, desc: 'Angka Kematian Bayi' },
    { key: 'cakupanAnc',      icon: '🏥', higherIsBetter: true,  desc: 'Kunjungan ANC K4' },
    { key: 'persalinanNakes', icon: '👩‍⚕️', higherIsBetter: true,  desc: 'Persalinan Nakes' },
    { key: 'cakupanImun',     icon: '💉', higherIsBetter: true,  desc: 'Imunisasi Dasar Lengkap' },
    { key: 'stunting',        icon: '📊', higherIsBetter: false, desc: 'Prevalensi Stunting' },
  ];
  wrap.innerHTML = items.map(({ key, icon, higherIsBetter }) => {
    ...
    return `
      <div class="kpi-card ${cls}" style="cursor:default">
        <div class="kpi-header">
...
function renderLaporan() {
  ...
  const statCards = [
    ...
    { label: 'Total Kasus Minggu Ini',value: totalKasus.toLocaleString('id-ID'), icon: '🦠', color: 'warning' },
  ...
  wrap.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:var(--space-4);margin-bottom:var(--space-6)">
      ${statCards.map(i => `
        <div class="kpi-card ${i.color}" style="cursor:default">
    ...
    <div style="padding:var(--space-6);background:var(--color-surface);border-radius:var(--radius-lg);border:1px solid var(--color-border);margin-bottom:var(--space-6)">
      <div style="font-size:var(--text-sm);font-weight:600;margin-bottom:var(--space-3);color:var(--color-text-muted);text-transform:uppercase;letter-spacing:0.05em">Catatan Sistem</div>
      <p style="font-size:var(--text-sm);line-height:1.75;color:var(--color-text);max-width:72ch">
        Data diperbarui setiap hari pukul 06.00 WIT dari <strong>${total} kabupaten/kota</strong>.
        ...
        Laporan lengkap dapat diunduh dalam format PDF atau Excel melalui tombol ekspor di bawah.
      </p>
      <div style="margin-top:var(--space-4);display:flex;gap:var(--space-3);flex-wrap:wrap">
        <button class="btn btn-primary btn-sm" onclick="alert('Fitur ekspor PDF akan segera tersedia.')">
          <i data-lucide="file-text" width="14" height="14"></i> Ekspor PDF
        </button>
        <button class="btn btn-outline btn-sm" onclick="alert('Fitur ekspor Excel akan segera tersedia.')">
          <i data-lucide="table" width="14" height="14"></i> Ekspor Excel
        </button>
      </div>
    </div>`;

  // Re-init lucide icons inside newly injected HTML
...
function renderPeta() {
  ...
  if (!wrap) return;
  wrap.innerHTML = `
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:420px;gap:var(--space-4);color:var(--color-text-muted);text-align:center">
      <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        <path d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/>
      </svg>
      <div style="font-weight:600;font-size:var(--text-lg)">Peta GIS Interaktif</div>
      <div style="font-size:var(--text-sm);max-width:320px;line-height:1.65">
        Visualisasi peta Papua dengan layer data kesehatan per kabupaten.
        Integrasi Leaflet.js + GeoJSON Papua tersedia di versi berikutnya.
      </div>
...
function initDashboard() {
  ...
  // Static tables & components — always rendered up-front
  renderKPI();
  renderAlerts();
  renderSurveilansTable();
  renderStokObat();
  ...
  // Charts — deferred 100ms to let layout settle
  setTimeout(() => {
...
// ============================================================
// BOOTSTRAP — exposed after DOMContentLoaded
// ============================================================
